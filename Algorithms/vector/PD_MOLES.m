function [solution, obj_values] = PD_MOLES(W1, Xtrain, ytrain, Xtest, ytest, num_iters, batch_size, sub_batch_size, eta_coef, eta_exp, lambda_coef, gamma_coef, radius, loss_fn, subgrad_fn, lmo_fn, gap_fn, print_freq, IS_CALCULATING_REGRET, optimal_value, stoptime)
    ...
    sum_grad_regularizer = zeros(size(W1));
    sum_stoch_subgrad = zeros(size(W1));
    ...
    for t = 1 : num_iters
        ...
        % LMO
        sum_grad_regularizer = sum_grad_regularizer + (t / lambda) * (Y - Y_tilde);
        Z = lmo_fn(sum_grad_regularizer);
        % sample an index
        ...
        sum_stoch_subgrad = sum_stoch_subgrad + stoch_subgrad;
        Z_tilde = Z1_tilde - (sum_stoch_subgrad - sum_grad_regularizer) ./ (t * beta);
