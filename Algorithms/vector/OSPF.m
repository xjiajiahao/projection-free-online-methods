function [solution, obj_values] = OSPF(W1, Xtrain, ytrain, Xtest, ytest, num_iters, batch_size, perturbation, IS_NON_SMOOTH, loss_fn, grad_fn, lmo_fn, gap_fn, print_freq, IS_ADVERSARIAL, IS_CALCULATING_REGRET, optimal_solution, stoptime)
    % initialization
    ...
    if IS_NON_SMOOTH
        ...
        block_size = ceil(nthroot(num_iters, 3));
    end

    delta = perturbation * (sqrt(numel(W) * num_iters / block_size) *  block_size);

    for t = 1 : num_iters
        W_old = W;
        if mod(t, block_size) == 0
            W(:) = 0;  % W = 0
            for i = 1 : block_size
