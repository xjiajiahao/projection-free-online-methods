function plot_CF(dataset)
    ...
    figure_names = {};
    curve_styles = {'-v', '-s', '-o', '-+'};
    stoch_methods = {'OFWRG', 'OSFW', 'P_MOLES', 'PD_MOLES'};
    stoch_method_names = {'R-OFWRG', 'OSFW', 'P-MOLES', 'PD-MOLES'};
    methods_with_stoch_grad = {'OFWRG', 'OSFW', 'P_MOLES', 'PD_MOLES'};
    ...
    if strcmp(dataset, 'Jester1')
        title_name = '';
        max_num_rounds = 3e3;
        max_per_round_time = 0.053;
        sub_batch_grid = [64, 1, 4, 16];
    elseif strcmp(dataset, 'MovieLens100K')
        title_name = '';
        max_num_rounds = 3e3;
        max_per_round_time = 0.028;
        sub_batch_grid = [64, 1, 4, 16];
    elseif strcmp(dataset, 'MovieLens1M')
        dataset = 'MovieLens1M';
        title_name = 'MovieLens 1M';
        max_num_rounds = 3e3;
        max_per_round_time = 0.65;
        sub_batch_grid = [64];
    elseif strcmp(dataset, 'Jester3')
        title_name = 'Jester 3';
        max_num_rounds = 3e3;
        max_per_round_time = 0.3;
        sub_batch_grid = [64];
    end
    ...
    for k = 1 : length(sub_batch_grid)
        ...
        if k == 1
            file_name = [ROOT, 'results_', dataset, '_CF_', num2str(sub_batch_grid(k)), '_1_regret.mat'];
            load(file_name);
            ...
            for i = 1 : num_trials
                file_name = [ROOT, 'results_', dataset, '_CF_', num2str(sub_batch_grid(k)), '_', num2str(i), '_regret.mat'];
                load(file_name);
        ...
        fig_name =[OUTPUT_DIR, dataset, '_CF_per_round_time_b=', num2str(sub_batch_grid(k))];
        ...
        % plot regret curves
        the_figure = figure('position', [0, 0, width, height]);
        fig_name =[OUTPUT_DIR, dataset, '_CF_regret_b=', num2str(sub_batch_grid(k))];
        figures{end+1} = the_figure;
        figure_names{end+1} = fig_name;
