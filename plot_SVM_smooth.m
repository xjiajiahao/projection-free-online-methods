...
figures = {};
figure_names = {};
curve_styles = {'-v', '-s', '-+', '-o', '-p', '-.'};
adv_methods = {'Meta_OFWRG', 'Meta_FW', 'PFOCO_SG', 'ROFW', 'OSPF', 'SFPL'};
adv_method_names = {'Meta OFWRG', 'Meta-Frank-Wolfe', 'PFOCO-SG', 'Regularized OFW', 'OSPF', 'Sampled FPL'};
sub_batch_grid = [1, 2, 4, 8];
methods_with_stoch_grad = {'Meta_OFWRG', 'Meta_FW', 'PFOCO_SG', 'MUSOFW', 'SHCGM', 'P_MOLES', 'PD_MOLES'};
...
for k = 1 : length(sub_batch_grid)
    ...
    if k == 1
        file_name = [ROOT, 'results_', dataset, '_SVM_smooth_1_1_regret.mat'];
        load(file_name);
        ...
        for i = 1 : num_trials
            file_name = [ROOT, 'results_', dataset, '_SVM_smooth_1_', num2str(i), '_regret.mat'];
            load(file_name);
        ...
        file_name = [ROOT, 'results_', dataset, '_SVM_smooth_', num2str(sub_batch_grid(k)), '_1_regret.mat'];
        load(file_name);
        len_cell_arr = length(obj_values_cell);
        for j = 1 : len_cell_arr
        ...
        for i = 1 : num_trials
            file_name = [ROOT, 'results_', dataset, '_SVM_smooth_', num2str(sub_batch_grid(k)), '_', num2str(i), '_regret.mat'];
            load(file_name);
            for j = 1 : len_cell_arr
                target_idx = 0;
    ...
    fig_name =[OUTPUT_DIR, dataset, '_SVM_smooth_per_round_time_b=', num2str(sub_batch_grid(k))];
    ...
    % plot regret curves
    the_figure = figure('position', [0, 0, width, height]);
    fig_name =[OUTPUT_DIR, dataset, '_SVM_smooth_regret_b=', num2str(sub_batch_grid(k))];
    figures{end+1} = the_figure;
    figure_names{end+1} = fig_name;
